...
function wizardItem(x,pos){const c=S.creos.find(z=>z.id===x.creo_id);if(!c)return"";const vv=videoVersions(c);let cur=vv.find(v=>v.path===x.source_path);
  ...
  return '<div class="dwitem" data-di="'+pos+'"><div class="dwprev">'+(cur&&cur.url?'<video controls playsinline preload="none" src="'+esc(cur.url)+'"></video>':'<div class="na">🎬</div>')+'</div><div class="dwbody"><b>'+(pos+1)+'. '+esc(x.title||c.result_caption||c.caption||("Видео #"+c.id))+'</b>'+
...
function batchCard(b){const active=!['sent','cancelled'].includes(b.status);const lvl=[...new Set((b.items||[]).map(i=>i.uniq_level))];const previews=(b.items||[]).map(i=>'<div class="bp">'+(i.clip_url?'<video muted loop playsinline preload="none" data-lazy="'+esc(i.clip_url)+'"></video>':i.poster_url?'<img loading="lazy" src="'+esc(i.poster_url)+'">':'🎬')+'</div>').join("");
